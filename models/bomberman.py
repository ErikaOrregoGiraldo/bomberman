...
class Bomberman(Agent):
    ...
    def move(self):
        ...
        if exit_position is not None:
            ...
            if self.path:
                next_position = self.path[0]  # Mantenemos la próxima posición en lugar de eliminarla
                if self.model.grid.is_cell_empty(next_position):  # Verificamos si la celda está vacía
                    self.model.grid.move_agent(self, next_position)
                    self.pos = next_position  # Actualiza la posición de Bomberman aquí
                    self.path.pop(0)  # Elimina la posición del camino solo si el movimiento fue exitoso
                else:
                    print(f"Posición {next_position} ocupada. No se puede mover.")
